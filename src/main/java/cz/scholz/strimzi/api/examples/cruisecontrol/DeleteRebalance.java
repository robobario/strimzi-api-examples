...
public class DeleteRebalance {
    private static final Logger LOGGER = LoggerFactory.getLogger(DeleteRebalance.class);
    ...
    public static void main(String[] args) {
        try (KubernetesClient client = new KubernetesClientBuilder().build()) {
            ...
            KafkaRebalanceBuilder kafkaRebalanceBuilder = new KafkaRebalanceBuilder()
                    .editMetadata().withName(REBALANCE_NAME).addToLabels("strimzi.io/cluster", "my-cluster")
                    .withClusterName("my-cluster").endMetadata().withNewSpec().endSpec();
            Crds.kafkaRebalanceOperation(client).inNamespace(NAMESPACE)
                    .resource(kafkaRebalanceBuilder.build()).delete();
        }
    }
}